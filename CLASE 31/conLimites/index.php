...
<body>		
	...
	else /* Hay datos en el GET */
	{
		...
		if (validarDatos())
		{
			...
			{
				$consulta = crearConsulta($_GET["parametro"], $_GET["orden"], 0);
				$pagina = 0;
			}
		}
		else
		{
			if(validarPagina())
			{
				$consulta = "SELECT * FROM alumnos LIMIT " . $_GET["pagina"] * 5 . ", 5;";
				$pagina = $_GET["pagina"];
			}
			else
			{
	...
	<center>
		...
		<a class="btn btn-success" href=index.php?<?=urlActual()?>&pagina=<?=getAnteriorBoton($pagina)?>>Anterior</a>
		<a class="btn btn-success" href=index.php?<?=urlActual()?>&pagina=<?=getSiguienteBoton($pagina)?>>Siguiente</a>
